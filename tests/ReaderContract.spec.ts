...
describe('ReaderContract', () => {
    ...
    it('should check merkle proof', async () => {
        ...
        .map((prb) => {
          // TODO: split big data
          const cells = prb
          .map(proofPart => beginCell().storeBuffer(proofPart.subarray(0, 32), Math.min(proofPart.length, 32)))
        ...
        const callback = await readerContract.sendVerifyProof(increaser.getSender(), {
          value: toNano('5.5'),
            receipt: cell,
            rootHash: beginCell()
            .storeBuffer(block.receiptTrie)
            .endCell(),
            path: beginCell().storeBuffer(rlp.encode(toNumber(receipt.transactionIndex))).endCell(),
...
const json = {
    "receipt": {
      ...
      "logs": [
        ...
        },
        {
          ...
          "topics": [
            "0x09a9af46918f2e52460329a694cdd4cd6d55354ea9b336b88b4dea59914a9a83"
          ]
        }
      ],
      "logsBloom": "",
      "status": "0x1",
      "type": "0x2"
    },
    "txProof": [
    ...
    "blockHeader": ""
  };
